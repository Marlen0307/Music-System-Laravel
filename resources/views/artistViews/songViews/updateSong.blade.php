...
@section('content')
    ...
    <div class="flex justify-center">
        <div class="w-8/12  sm:w-6/12 md:w-4/12 bg-white p-6 rounded shadow my-10">
            ...
            @endif
            <form method="post" action="{{route("updateSong", $song)}}" enctype="multipart/form-data">
                @csrf
                @method("PUT")
                <div class="mb-4" >
                ...
                @if($albums->count())
                    ...
                    w-full p-4 rounded ">
                            ...
                            @foreach($albums as $album)
                                <option @if($album->id == $song->album_id) selected @endif
                        ...
                        @foreach($users as $user)
                            <div class="mx-1">
                                @if($user->isArtist() && auth()->user()->id != $user->id)
                                    <input @if($selectedArtists->contains($user->id)) checked @endif type="checkbox" name="artists[]" value="{{$user->id}}">
                        ...
                        @foreach($users as $user)
                            <div class="mx-1">
                                @if($user->isSongwriter())
                                    <input
                                        @foreach($selectedSongwriters as $songwriter)
                                           @if($songwriter->songwriter == $user->firstname .' '. $user->lastname) checked @endif
                                        @endforeach
                                    type="checkbox" name="songwriters[]" value="{{$user->firstname ." ". $user->lastname}}">
                                    <label for="{{$user->id}}">{{$user->firstname ." ". $user->lastname}}</label>
                                @endif
                            </div>
                        @endforeach
